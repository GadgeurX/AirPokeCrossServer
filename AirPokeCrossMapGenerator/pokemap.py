...
def main():
  ...
  offsets = calculate_map_offsets(banks, 3, 1)
  ...
  output_file.write(str(int(width / 16)) + " ") 
  output_file.write(str(int(height / 16)) + "\n")
  x_orig = min_x * 16
  y_orig = min_y * 16
  ...
  for y in range(height):
    for x in range(width):
      ...
      #debug('tile at ({}, {}): {:#x}, attribute: {:#x}'.format(
        #x, y, tile, attribute))
      tile_sprites[(x, y)] = (tile, attribute)
